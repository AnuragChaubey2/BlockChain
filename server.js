...
app.get('/address/:address', async (req, res) => {
    ...
    try {
      const etherscanAPIKey = '4CR5HA7W5VT9NYD9IGDV8B9419DJH5H4VN';
      ...
      res.json({ address, ethBalance, erc20TokenBalances, transactions, alerts });
    } catch (error) {
...
app.post('/alerts', async (req, res) => {
  const { address, thresholdAmount } = req.body;
  ...
  try {
    ...
    if (existingAlert) {
      ...
      const newAlert = new Alert({ address, thresholdAmount });
      await newAlert.save();
